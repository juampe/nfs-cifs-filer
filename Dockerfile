...
RUN wget --no-check-certificate https://github.com/just-containers/s6-overlay/releases/download/v2.1.0.0/s6-overlay-${TARGETARCH}.tar.gz -O /tmp/s6-overlay.tar.gz && tar xfz /tmp/s6-overlay.tar.gz -C / && rm -f /tmp/*.tar.gz 
